    ...
    <div class="wrap">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px">
          ...
          <div style="display:flex;gap:32px;text-align:right;padding-left:8px">
            <div>
              ...
              <div style="display:flex;align-items:center;gap:2px;font-size:18px;font-weight:600;color:var(--t1);margin-left:-4px">
                <span v-html="COIN_SVG" style="display:inline-flex;vertical-align:middle;width:14px;height:14px"></span>
                {{ state.hero.coin.toLocaleString() }}
              </div>
            </div>
            <div>
              <div style="font-size:12px;color:var(--t3);margin-bottom:2px;">储蓄</div>
              <div style="display:flex;align-items:center;justify-content:flex-start;gap:2px;font-size:18px;font-weight:600;color:var(--t1)">
                <span style="color:#d4a017;font-size:16px;font-weight:bold">¥</span>
                {{ (state.hero.realMoney || 0).toLocaleString() }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="mt24">
        ...
        <div class="card cp">
          <div class="heatmap-scroll">
            <div class="heatmap-inner">
              <div class="heatmap-months">
                <div v-for="(col, i) in heatmapData.cols" :key="i" class="hm-ml">
                  {{ i === 0 || col[0].getMonth() !== heatmapData.cols[i-1][0].getMonth() ? ['1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月'][col[0].getMonth()] : '' }}
                </div>
              </div>
              <div class="heatmap-body">
                ...
                <div v-for="(col, i) in heatmapData.cols" :key="i" class="hm-col">
                  <div v-for="(d, j) in col" :key="j" class="hm-cell"
                    :data-l="heatmapData.lv(heatmapData.countMap[heatmapData.localDateStr(d)] || 0)"
                    :data-tip="(heatmapData.countMap[heatmapData.localDateStr(d)] || 0) + ' 次历险 · ' + d.toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })"
          ...
          <div class="hm-legend">
            <span>少</span>
            <div class="hm-lc" style="background:var(--h0)"></div>
            <div class="hm-lc" style="background:var(--h1)"></div>
            <div class="hm-lc" style="background:var(--h2)"></div>
            <div class="hm-lc" style="background:var(--h3)"></div>
            <div class="hm-lc" style="background:var(--h4)"></div>
            <span>多</span>
          </div>
        </div>
      </div>
      ...
      <div class="mt24">
        ...
        <div class="card cp">
          <div v-if="!recentActivity.length" class="empty">
            <div class="empty-icon">🌿</div>开始你的第一次历险吧
          </div>
          <div v-else>
            <div v-for="item in recentActivity" :key="item.ts" class="activity-item">
              ...
              <div style="flex:1">
                ...
                <div style="font-size:11px;color:var(--t3);font-family:monospace">
                  {{ fmtDate(item.data.date) }} · {{ item.type === 'adv' ? '+' + item.data.xp + ' XP' : '随笔' }}
                </div>
              </div>
            </div>
          </div>
...
const { state, XP_TABLE, COIN_SVG, DAILY_QUOTES, getLevel, getLevelTitle, fmtDate } = useStore()
